...
# EXTRACT PREDICTORS FOR EVERY MGRS CELL IN MADRID USING A 5KM BUFFER
...
for (i in 1:nrow(mgrs_df)) {
  ...
  # buffer
  pol1 <- vect(mgrs_df[i,], geom=c('x','y'), 'epsg:4326') %>% buffer(width=5000)
...
# save data.frame
write.table(mgrs_df, 'results/mgrs_pred_5km.txt')
